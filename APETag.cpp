...
/**
* Oplus 2011-01-10 zhouhy Add
* This function is for retrieve wchar length.
* @param s1
*   [IN]     wchar string
* @return
*   wchar string length
*/
size_t CAPETagField::ApeWcslen(const str_utf16* s)
{
    const str_utf16* orig_s = s;
...
{
    // field name
    m_spFieldNameUTF16.Assign(new str_utf16 [ApeWcslen(pFieldName) + 1], TRUE);
    memcpy(m_spFieldNameUTF16, pFieldName, (ApeWcslen(pFieldName) + 1) * sizeof(str_utf16));
...
{
    m_spIO.Assign(new CStdLibFileIO);
/*Oplus 2011-7-4 zouf Add begin for don't analyze when file can't open */
//    m_spIO->Open((LPCTSTR)pFilename);
    if (m_spIO->Open((LPCTSTR)pFilename) >= 0) {
/*Oplus 2011-7-4 zouf Add end  */
...
/*Oplus 2011-7-4 zouf Add begin for  don't Analyze*/
    } else {
        m_bAnalyzed = TRUE;
        m_nFields = 0;
        m_nTagBytes = 0;
...
/**
* Oplus 2011-01-10 zhouhy Add
* This function is for compare wchar string.
* @param s1
*   [IN]   wchar string
* @param s2
*   [IN]   wchar string
...
*   > 0        s1 > s2
*/
int CAPETag::ApeWcscmp(const str_utf16* s1, const str_utf16* s2)
{
    if ( s1 == s2 )
    {
...
int CAPETag::GetTagFieldIndex(const str_utf16 * pFieldName)
{
    ...
    for (int z = 0; z < m_nFields; z++)
    {
        if (ApeWcscmp(m_aryFields[z]->GetFieldName(), pFieldName) == 0)
            return z;
    }
...
int CAPETag::CreateID3Tag(ID3_TAG * pID3Tag)
{
    ...
    pID3Tag->Track = (unsigned char)atoi((char *)cBuffer);//(unsigned char) _wtoi(cBuffer);
...
int CAPETag::LoadField(const char * pBuffer, int nMaximumBytes, int * pBytes)
{
    ...
    // size and flags
    int nLocation = 0;
    int nFieldValueSize = pBuffer[nLocation];//*((int *) &pBuffer[nLocation]);
    nLocation += 4;
    int nFieldFlags = pBuffer[nLocation];//*((int *) &pBuffer[nLocation]);
    nLocation += 4;
...
int CAPETag::SetFieldID3String(const str_utf16 * pFieldName, const char * pFieldValue, int nBytes)
{
    ...
    // set the field
    //fix bug 438032,stagefrightMediadataRetriver will handle character encode.
    SetFieldString(pFieldName, spBuffer, TRUE);
