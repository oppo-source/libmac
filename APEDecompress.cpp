...
int CAPEDecompress::InitializeDecompressor()
{
    ...
    #else
    m_cbFrameBuffer.CreateBuffer( min(DECODE_BLOCK_SIZE *10, (GetInfo(APE_INFO_BLOCKS_PER_FRAME) + DECODE_BLOCK_SIZE)) * m_nBlockAlign, m_nBlockAlign * 64);
    #endif
...
int CAPEDecompress::GetData(char * pBuffer, int nBlocks, int * pBlocksRetrieved)
{
    ...
    while ((nBlocksLeft > 0) && (nBlocksThisPass > 0))
    {
        ...
        #if 1
        if (nFrameBufferBlocks <= 0 && m_cbFrameBuffer.MaxGet() > 0) {
            nBlocksThisPass = nBlocksLeft < m_cbFrameBuffer.MaxGet()?nBlocksLeft:m_cbFrameBuffer.MaxGet();

            if(nBlocksThisPass > 8192){
                nBlocksThisPass = 8192;
            }
        }
        #endif
        ...
        if (nBlocksThisPass > 0) {
            ret = m_cbFrameBuffer.Get(pOutputBuffer, nBlocksThisPass * m_nBlockAlign);
            pOutputBuffer += nBlocksThisPass * m_nBlockAlign;//ret;
            nBlocksLeft -= nBlocksThisPass;//ret/m_nBlockAlign;
            m_nFrameBufferFinishedBlocks -= nBlocksThisPass;//ret/m_nBlockAlign;
        }
    }
...
int CAPEDecompress::Seek(int nBlockOffset)
{
    ...
    nBlocksToSkip = nBlocksToSkip < 8192 ?nBlocksToSkip:8192;
...
long CAPEDecompress::GetInfo(APE_DECOMPRESS_FIELDS Field, long nParam1, int nParam2)
{
    long nRetVal = 0;
    BOOL bHandled = TRUE;
...
//                WAVEFORMATEX wfeFormat; GetInfo(APE_INFO_WAVEFORMATEX, (int ) &wfeFormat, 0);
    ...
    ALOGV("getInfo nRetVal %ld",nRetVal);
    ALOGV("getInfo this address:%p",this);
    return nRetVal;
}
