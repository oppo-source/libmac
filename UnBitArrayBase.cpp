...
CUnBitArrayBase * CreateUnBitArray(IAPEDecompress * pAPEDecompress, int nVersion)
{
    ...
    if (nFurthestReadByte > 0)
    {
        ...
        CAPETag * pAPETag = (CAPETag *)(intptr_t) pAPEDecompress->GetInfo(APE_INFO_TAG);
        if (pAPETag != NULL)
...
int CUnBitArrayBase::FillBitArray()
{
    ...
    if (m_nFurthestReadByte > 0)
    {
        ...
        //Xiaopeng.Hu@PSW.MultiMedia.MediaServer.OplusMedia.1023547, 2017/06/06,
        //Modify for abnormal ape make crash.
        if ((nFurthestReadBytes > 0) && (nBytesToRead > nFurthestReadBytes)) {
