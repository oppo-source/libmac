...
CAPELink::CAPELink(const char * pFilename)
{
    ...
    // open the file
    CStdLibFileIO ioLinkFile;
    if (ioLinkFile.Open((LPCTSTR)pFilename) == ERROR_SUCCESS)
    {
        // create a buffer
...
//                CSmartPtr<char> spImageFile(cImageFile, TRUE);

                // process the path
                if (strchr(cImageFile, '\\') == NULL)
                {
                    char cImagePath[MAX_PATH + 1];
                    strcpy(cImagePath, pFilename);
                    strcpy(strchr(cImagePath, '\\') + 1, cImageFile);
