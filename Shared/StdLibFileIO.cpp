...
int CStdLibFileIO::Open(const LPCTSTR pName)
{
    Close();
...
//begin add by jiangyg
    else if(strstr(pName, "sharedfd://"))
    {
         sscanf(pName, "sharedfd://%d:%d:%d", &fd, &offset, &length);
       m_pFile = fdopen(fd, "r");
    }
//end add
    else
...
int CStdLibFileIO::Create(const LPCTSTR pName)
{
    Close();
